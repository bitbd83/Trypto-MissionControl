...
class AffiliateCell extends React.Component {
  state = {
    anchorEl: null,
    confirm: false,
    action: null,
    ...
    return (
          <div key={index} className="contact-item py-3">
            <div className="col text-truncate">
              <Typography variant="subheading" className="text-truncate" component={Link} to={`${this.props.path}/${item.id}`}>
                {item.name}
              </Typography>
              <Typography variant="caption" className="text-truncate">
                {item.trackingCodesCount} <IntlMessages id="pages.AffiliatesPage.trackingCodes.label.title"/>
              </Typography>
            </div>
            ...
            <div className="col-auto px-1 actions d-none d-sm-flex">
              ...
              >
                ...
                {!Boolean(item.isArchived) ? [
                  ...
                  <MenuItem key={'details'} component={Link} to={`${this.props.path}/${item.id}`}>
                    <IntlMessages id="content.dropdown.menu.details"/>
                  </MenuItem>,
                  (!Boolean(item.active) ? (
