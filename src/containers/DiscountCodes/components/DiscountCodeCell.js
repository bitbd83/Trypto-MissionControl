...
class DiscountCodeCell extends React.Component {
  state = {
    anchorEl: null,
    confirm: false,
    action: null,
  ...
  structureFee = (code, usefactor = false) => {
    if(!code && code !== 0) {
      return false;
    }
    if(!usefactor & code === 0) {
    ...
    if(item.tieredRateSetup.strategy === 'Flat' || !usefactor){
      return (
        <TenantsCurrencyVal data={tenantsByDomain} value={code}/>
      )
    } else {
      return code + "%";
    }
          ...
          <div key={index} className="contact-item py-3">
            ...
            <div className="col-sm-3 px-0 d-none d-md-flex flex-column">
              <Typography variant="body2">
                {this.structureFee(item.tieredRateSetup.rates[0].rate, true)} Off
              </Typography>
            </div>
...
export default DiscountCodeCell;
