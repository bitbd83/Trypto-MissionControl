...
class Header extends React.Component {
  ...
  render() {
    const { drawerType, locale, navigationStyle, horizontalNavPosition, tenantsByDomain, authUser } = this.props;
      ...
      <AppBar className={`app-main-header ${navigationStyle === HORIZONTAL_NAVIGATION && horizontalNavPosition === BELOW_THE_HEADER ? 'app-main-header-top' : ''}`}>
        <Toolbar className="app-toolbar" disableGutters={false}>
          ...
          <Link className="app-logo mr-2 d-none d-sm-block" to="/">
            <img
              src={tenantsByDomain && Object.keys(tenantsByDomain).length > 0 && tenantsByDomain.logo && authUser !== null ? tenantsByDomain.logo.imageUrl : ''}
              alt={tenantsByDomain.tenantName}
              title={tenantsByDomain.tenantName}
            />
          </Link>
          ...
          <ul className="header-notifications list-inline ml-auto">
            ...
            <li className="list-inline-item app-tour">
              <Dropdown className="quick-menu" isOpen={this.state.appNotification} toggle={this.onAppNotificationSelect.bind(this)}>
                <DropdownToggle className="d-inline-block" tag="span" data-toggle="dropdown">
            ...
            <li className="list-inline-item mail-tour">
              <Dropdown className="quick-menu" isOpen={this.state.mailNotification} toggle={this.onMailNotificationSelect.bind(this)}>
                <DropdownToggle className="d-inline-block" tag="span" data-toggle="dropdown">
                  <IconButton className="icon-btn size-20 font-size-20">
...
const mapStateToProps = ({ settings, tenants, auth }) => {
  const { drawerType, locale, navigationStyle, horizontalNavPosition } = settings;
  const { tenantsByDomain } = tenants;
  const { authUser } = auth;
  return { drawerType, locale, navigationStyle, horizontalNavPosition, tenantsByDomain, authUser };
};
